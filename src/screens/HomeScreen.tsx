...
const HomeScreen = ({ route, navigation }: any) => {
  ...
  const handleRegister = async (id: string) => {
    ...
    try {
      ...
      setEvents(prev => prev.map(ev =>
        ev.id === id
          ? { ...ev, isRegistered: !ev.isRegistered, registrationCount: ev.isRegistered ? ev.registrationCount - 1 : ev.registrationCount + 1 }
          : ev
      ));
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      {/* FIX: Added contentContainerStyle with paddingBottom so content is not clipped */}
      <ScrollView
      ...
      >
        ...
        {subscribedFeedEvents.length > 0 ? (
          // FIX: Replaced FlatList with horizontal ScrollView to avoid nested scroll conflicts
          <ScrollView
            horizontal
            showsHorizontalScrollIndicator={false}
            contentContainerStyle={{ paddingLeft: 20 }}
          >
            {subscribedFeedEvents.map((item) => (
              <TouchableOpacity
                key={item.id}
...
const styles = StyleSheet.create({
  ...
  // FIX: Removed flex:1 from container so ScrollView content can grow beyond screen height
  container: { backgroundColor: '#000b18' },
  // FIX: paddingBottom ensures last item is not hidden behind nav bars
