...
const ProfileScreen = ({ route, navigation }: any) => {
  ...
  const handleSave = async () => {
    setMessage('');
    if (name.trim().length < 2) { setMessage('Name must be at least 2 characters.'); return; }
    if (!email.trim()) { setMessage('Email is required.'); return; }
    if (isOrganizer && organizationName.trim().length < 2) {
      setMessage('Organization name must be at least 2 characters.'); return;
    }
    const payload: any = { fullName: name.trim(), email: email.trim() };
    if (isStudent && studentId.trim()) payload.studentId = studentId.trim();
    if (isOrganizer) payload.organizationName = organizationName.trim();
        ...
        <View style={styles.form}>
          ...
          {isOrganizer && (
            <>
              ...
              <TextInput style={styles.input} value={organizationName} onChangeText={setOrganizationName} placeholder="Enter organization name" placeholderTextColor="rgba(255,255,255,0.3)" />
            </>
          )}
          ...
          {isStudent && (
            <>
              ...
              <TextInput style={styles.input} value={studentId} onChangeText={setStudentId} placeholder="Enter your student ID" placeholderTextColor="rgba(255,255,255,0.3)" />
            </>
          )}

          <Text style={styles.label}>Email</Text>
          <TextInput style={styles.input} value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none" placeholder="Enter your email" placeholderTextColor="rgba(255,255,255,0.3)" />
        </View>
...
const styles = StyleSheet.create({
  ...
  headerRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, marginTop: 20, marginBottom: 20 },
